...
class OffscreenRenderer:
    def __init__(self, data: TrackingDataProvider, config: dict) -> None:
        ...
        self._set_camera(self.geometry_plotter, data, camera)
        ...
        self._set_camera(self.annotation_plotter, data, camera)
    ...
    @staticmethod
    def _set_camera(plotter, data: TrackingDataProvider, camera) -> None:
        if camera is None:
            frame_camera(plotter, data.room_bounds())
        else:
...
def run_export(config: dict, logger: EventLogger) -> None:
    ...
    for body_name, body_type in config.get("body_types", {}).items():
        data.set_body_type(body_name, body_type)
